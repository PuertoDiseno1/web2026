...
async function main() {
  ...
  const projects = [
    ...
    [8, "woc-stand", "WOC Stand", "Chile se proyecta al mundo", "Acompanamos a Wines of Chile en ProWein Dusseldorf con el desafio de traducir la identidad del pais en una experiencia espacial.", "Diseno Stand\nSupervision Implementacion", "Diseno industrial", "", false],
    [9, "accion-solidaria", "Accion Solidaria", "Conectamos por la misma causa", "A partir de la nueva arquitectura de marca trabajada para Hogar de Cristo, se desarrollo la nueva identidad visual para Accion Solidaria.", "Branding", "Branding", "", false],
  ];
  for (const [order, slug, title, subtitle, description, services, categories, videoEmbed, featured] of projects) {
    await prisma.project.upsert({
      ...
      create: {
        ...
        featured: Boolean(featured), published: true,
      },
    });
  }
